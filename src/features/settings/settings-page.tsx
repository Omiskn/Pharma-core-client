            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-right">
                <div className="space-y-2">
                  ...
                  <Input id="api-base-url" value={apiConfig.baseUrl || window.location.origin} readOnly className="text-right" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="api-prefix">بادئة واجهة البرمجة</Label>
                  <Input id="api-prefix" value={apiConfig.prefix} readOnly className="text-right" />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-right">
                <div className="space-y-2">
                  <Label htmlFor="api-health-path">مسار الفحص</Label>
                  <Input id="api-health-path" value={apiConfig.healthPath} readOnly className="text-right" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="api-status">حالة الاتصال</Label>
                  <Input
                    id="api-status"
                    value={apiHealth.isLoading ? "جار التحقق" : apiHealth.data ? "متصل" : "غير متصل"}
                    readOnly
